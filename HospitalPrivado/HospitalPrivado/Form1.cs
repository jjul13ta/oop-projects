...
namespace HospitalPrivado
{
    internal partial class Form1 : Form
    {
        ...
        {
            lbl_GananciaHosp.Text = "Recaudacion total del hospital = " + Hospital.Ganancia.ToString();
        }
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e) // Carga especialidades
        {
            CargaEspecialidad frm = new CargaEspecialidad(Hospital);
            frm.Show();
        }

        private void cargaMedicosToolStripMenuItem_Click(object sender, EventArgs e) // Carga medicos
        {
            CargaMedico frm = new CargaMedico(Hospital);
             frm.Show();
        }

        private void cargaPacientesToolStripMenuItem_Click(object sender, EventArgs e) // Carga pacientes
        {
            CargaPaciente frm = new CargaPaciente(Hospital);
             frm.Show();
        }

        private void cargaEstudiosToolStripMenuItem_Click(object sender, EventArgs e) // Carga estudios
        {
             CargaEstudio frm = new CargaEstudio(Hospital);
            frm.Show();
        }

        private void registroResultadosToolStripMenuItem_Click(object sender, EventArgs e) // Registro resultados
        {
             RegistroResultado frm = new RegistroResultado(Hospital);
            frm.Show();
        }

        private void vistaGeneralToolStripMenuItem_Click(object sender, EventArgs e) // Vista general
        {
            VistaGeneral frm = new VistaGeneral(Hospital);
            frm.Show();
        }

        private void btn_DescendienteHI_Click(object sender, EventArgs e)
        {
            if (txt_NamePaciente.Text == "")
            {
                MessageBox.Show("Ingrese el nombre del paciente.");
                return;
            }

            var historia = Hospital.Pacientes
                .Where(p => p.Nombre == txt_NamePaciente.Text)
                .SelectMany(p => p.HistoriaClinica.Consultas)
                ...
                .Select(c => new
                {
                    ...
                    Fecha = c.Fecha,
                    OrdenMedica = c.OrdenMedica != null,
                    Estudio = c.Estudio != null ? c.Estudio.Tipo : "Sin estudio",
                    Duracion = c.Duracion
                })
                .ToList();

            dataGridView1.DataSource = null;
            dataGridView1.DataSource = historia;
            ActualizarLBL();
        }

        private void btn_EspecialidadHI_Click(object sender, EventArgs e)
        {
            if (txt_NamePaciente.Text == "" || txt_NameEsp.Text == "")
            {
                MessageBox.Show("Ingrese el nombre del paciente y la especialidad.");
                return;
            }
            ...
            var historia = Hospital.Pacientes
                .Where(p => p.Nombre == txt_NamePaciente.Text)
                .SelectMany(p => p.HistoriaClinica.Consultas)
                .Where(c => c.Medico.Especialidades.Any(es => es.Nombre == txt_NameEsp.Text))
                .Select(c => new
                {
                    Medico = c.Medico.Nombre,
                    Fecha = c.Fecha,
...
=======
        ...
        {
            ConsultaMedica frm = new ConsultaMedica(Hospital);
            frm.Show();
        }

       
        private void btn_AscendenteEsp_Click(object sender, EventArgs e)
        {
            var lista = Hospital.Especialidades
            .OrderBy(es => es.Ganancia)
            .ToList();
        ...
        private void btn_DescendienteEsp_Click(object sender, EventArgs e)
        {
            var lista = Hospital.Especialidades
           .OrderByDescending(es => es.Pacientes)
            .ToList();
